...
**Dernière mise à jour :** 12 août 2026 — Session 16
...
cd backend
docker compose up -d
npm test              # 151 tests verts attendus
...
**Phase 3 en cours. AT-02 et US-14 clôturés (151 verts). Prochain jalon S16 : choisir et cadrer la prochaine US** (US-09 annulations / US-20 Socket.IO / US-11 remarque / US-10 couples-invités) **avant tout code** (RBAC, forme de réponse, taxonomie d'erreurs, stratégie de requête). Trancher aussi, à froid, le point ouvert **colonne générée vs trigger** (justification stale depuis le retrait d'`unaccent`).

Rappels de discipline : **lancer la suite avant tout commit `green`** ; **`grep`/`\d`/`git show` avant d'écrire un fait** ; **répondre au cadrage avant de coder** ; **consigner toute validation Diego datée** ; **clôture ≠ suite verte** (identifier les chemins non exercés et lesquels ont un appelant réel) ; réécriture d'historique **avant push** uniquement ; `parseISO(date + "T00:00:00Z")` pour toute date.

---

*Rapport synthétique — 15 sessions — dernière mise à jour 11 août 2026*
